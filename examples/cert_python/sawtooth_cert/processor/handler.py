...
from examples.cert_python.sawtooth_cert.processor.cert_payload import CertPayload
from examples.cert_python.sawtooth_cert.processor.cert_state import Game
from examples.cert_python.sawtooth_cert.processor.cert_state import CertState
from examples.cert_python.sawtooth_cert.processor.cert_state import CERT_NAMESPACE
...
class CertTransactionHandler(TransactionHandler):
    ...
    def apply(self, transaction, context):
        ...
        if cert_payload.action == 'delete':
            game = cert_state.get_game(cert_payload.name)

            if game is None:  # Error Checking
                raise InvalidTransaction(
                    'Invalid action: game does not exist')

            cert_state.delete_game(cert_payload.name)  # Trigger the action

        elif cert_payload.action == 'create':

            if cert_state.get_game(cert_payload.name) is not None:
                raise InvalidTransaction(
                    'Invalid action: Game already exists: {}'.format(
                        cert_payload.name))

            game = Game(name=cert_payload.name,
                        board="-" * 9,
                        state="P1-NEXT",
                        player1="",
                        player2="")

            cert_state.set_game(cert_payload.name, game)
            _display("Player {} created a game.".format(signer[:6]))

        elif cert_payload.action == 'take':
            game = cert_state.get_game(cert_payload.name)
            ...
            if game.board[cert_payload.space - 1] != '-':
                raise InvalidTransaction(
